...
e    'PK': {'uid': '3301828218', 'password': '3A0E972E57E9EDC39DC4830E3D486DBFB5DA7C52A4E8B0B8F3F9DC4450899571'},
...
async def fetch_token(region: str) -> Dict:
    ...
    # --- MODIFIED LINE ---
    # The "400 Bad Request" error happened because the 'password' parameter was missing.
    # It has been added here to fix the issue.
    url = f"https://atx-jwt-pi.vercel.app/token?uid={credentials['uid']}&password={credentials['password']}"
    
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            # Raise an exception for bad status codes (like the 400 you received)
...
def get_event_data():
    ...
    try:
        ...
        # This will catch errors from the token fetching part, like the 400 error you saw.
        return jsonify({"success": False, "error": f"Failed to fetch token: {e}"}), 502
